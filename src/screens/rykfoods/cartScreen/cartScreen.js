...
const CartScreen = ({navigation}) => {
  ...
  const setQuantity = (pro_id, quantity,pro_variation) => {
    if (quantity == 0) {
      remove(pro_id,pro_variation);
    } else {
      var filtered = products.filter(product => {
        if (pro_id == product.pro_id && pro_variation.id == product.variation.id) {
  ...
  const remove = (pro_id,pro_variation) => {
    var filtered = products.filter(product => {
      if (pro_id == product.pro_id && pro_variation.id == product.variation.id) {
      }else{
        return product;
            ...
            <View key={item.variation.price} style={styles.itemSection}>
              ...
              <View style={styles.itemDetail}>
                <Text style={styles.title}>
                  {item.pro_name}({item.variation.name})
                </Text>
                <View style={styles.row}>
                  <View style={styles.row1}>
                    <TouchableOpacity
                      onPress={() => {
                        if (item.quantity > 0) {
                          setQuantity(item.pro_id, item.quantity - 1,item.variation);
                        }
                      }}
                      style={styles.minus}>
        ...
        {products.length != 0 && (
          <View>
            <View style={styles.totalRow}>
              <Text style={[styles.totalText, {fontSize: 16}]}>Subtotal</Text>
              <Text style={[styles.totalText, {fontSize: 16}]}>
                Rs. {total}.00
              </Text>
            </View>
